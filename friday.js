...
const CFG = {
  ...
  provider:    localStorage.getItem('friday_provider') || 'gemini', // 'gemini' | 'openrouter'
...
window.addEventListener('load', () => {
  ...
  if (!CFG.geminiKey && !CFG.openrKey) openSettings();
...
function updateUplink() {
  ...
  const hasKey = CFG.geminiKey || CFG.openrKey;
  const label  = hasKey ? (CFG.provider==='openrouter' ? 'OPENROUTER' : 'GEMINI') : 'NO KEY';
  el.textContent  = hasKey ? 'ACTIVE' : 'NO KEY';
  el.title        = label;
  el.style.color  = hasKey ? 'var(--green)' : '#444';
  bar.style.width = hasKey ? '100%' : '8%';
  bar.style.background = hasKey ? 'var(--green)' : '#444';
  // update model badge
  const mb = document.getElementById('modelBadge');
  if (mb) mb.textContent = CFG.provider==='openrouter' ? CFG.openrModel.split('/')[1]?.split(':')[0]?.toUpperCase() || 'OPENROUTER' : 'GEMINI 2.5 FLASH';
}
...
function toggleProviderFields() {
  ...
  document.getElementById('geminiFields').style.display  = p==='gemini'     ? '' : 'none';
  document.getElementById('openrFields').style.display   = p==='openrouter' ? '' : 'none';
}
...
function saveSettings() {
  ...
  CFG.provider   = document.getElementById('providerSelect').value;
...
async function callFriday(text) {
  if (!CFG.geminiKey && !CFG.openrKey) {
    addMsg('friday','No API key configured, boss. Open Settings to add one.');
  ...
  try {
    let reply;
    if (CFG.provider === 'openrouter') reply = await callOpenRouter(text);
    ...
    if (msg.includes('429') || msg.toLowerCase().includes('rate')) {
      ...
      addMsg('friday',`Rate limit hit, boss. Cooling down 60s. ${CFG.provider==='gemini' ? 'Consider switching to OpenRouter in Settings for higher limits.' : 'Try a different model in Settings.'}`);
    } else {
...
async function testUplink() {
  try {
    const reply = await (CFG.provider==='openrouter' ? callOpenRouter('Say exactly: Uplink confirmed, boss.') : callGemini('Say exactly: Uplink confirmed, boss.'));
    // Remove the test messages from history
    history.splice(-2, 2);
    setStatus('ONLINE', true);
    addMsg('friday', reply);
    speakReply(reply);
