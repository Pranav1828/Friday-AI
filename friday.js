...
// ═══════════════════════════════════════════════════
const CFG = {
  openrKey:    localStorage.getItem('friday_openr_key') || '',
  openrModel:  localStorage.getItem('friday_openr_model') || 'google/gemma-3-27b-it:free',
...
const OR_MODELS = [
  { id:'google/gemma-3-27b-it:free',                 label:'Google Gemma 3 27B (free) ⭐ Recommended' },
  ...
  { id:'google/gemini-2.0-flash-exp:free',           label:'Gemini 2.0 Flash Exp (free) — Unstable' },
];
...
function updateUplink() {
  ...
  el.textContent  = CFG.openrKey ? 'ACTIVE' : 'NO KEY';
  el.style.color  = CFG.openrKey ? 'var(--green)' : '#444';
  bar.style.width = CFG.openrKey ? '100%' : '8%';
  bar.style.background = CFG.openrKey ? 'var(--green)' : '#444';
  const mb = document.getElementById('modelBadge');
  if (mb) mb.textContent = CFG.openrModel.split('/')[1]?.split(':')[0]?.toUpperCase() || 'OPENROUTER';
}
...
function openSettings() {
  document.getElementById('openrKeyInput').value    = CFG.openrKey;
  document.getElementById('hudColour').value        = CFG.hud;
  populateModelSelect();
  document.getElementById('openrModelSelect').value = CFG.openrModel;
  scanVoices();
...
function populateModelSelect() {
  const sel = document.getElementById('openrModelSelect');
  if (!sel) return;
  sel.innerHTML = '';
  OR_MODELS.forEach(m => {
  ...
  sel.value = CFG.openrModel;
}

function saveSettings() {
  CFG.openrKey   = document.getElementById('openrKeyInput').value.trim();
  CFG.openrModel = document.getElementById('openrModelSelect').value;
  CFG.voice      = document.getElementById('voiceSelect').value;
  const newHud   = document.getElementById('hudColour').value;
  localStorage.setItem('friday_openr_key',   CFG.openrKey);
  localStorage.setItem('friday_openr_model', CFG.openrModel);
  ...
  if (CFG.openrKey) {
    ...
    addMsg('friday','Testing OpenRouter uplink, boss. Stand by...');
    testUplink();
  }
}
...
async function callFriday(text) {
  if (!CFG.openrKey) {
    addMsg('friday','No OpenRouter API key configured, boss. Tap the arc reactor to add one from openrouter.ai/keys');
  ...
  try {
    ...
    if (msg.includes('429') || msg.toLowerCase().includes('rate')) {
      ...
      addMsg('friday','Rate limit hit, boss. Cooling down 60s. Try a different model in Settings.');
    } else {
      addMsg('friday',`Error, boss: ${msg}`);
    }
...
async function callOpenRouter(text) {
  ...
  // Try current model first, then fall through OR_MODELS list automatically
  const tryModels = [CFG.openrModel, ...OR_MODELS.map(m=>m.id).filter(id=>id!==CFG.openrModel)];
    ...
    try {
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers:{
          ...
          'Authorization': `Bearer ${CFG.openrKey}`,
          'HTTP-Referer': window.location.href,
          'X-Title': 'FRIDAY OS'
        },
        body: JSON.stringify({ model: modelId, messages, max_tokens:600, temperature:0.8, top_p:0.9 })
      });
      const data = await res.json();
      console.log(`[FRIDAY] ${modelId} → ${res.status}`, data.error?.message || 'OK');

      // Auth failure — no point trying other models
      if (res.status === 401 || res.status === 403) {
        throw new Error(`Invalid API key (${res.status}). Re-enter your key in Settings, boss.`);
      }
      // Daily limit hit
      if (res.status === 429) {
        throw new Error(`Daily request limit reached (${res.status}). Try again tomorrow or add credits at openrouter.ai`);
      }
      if (!res.ok) {
        const msg = data.error?.message || '';
        if (msg.toLowerCase().includes('provider') || msg.toLowerCase().includes('no endpoints') || res.status === 503 || res.status === 502) {
          console.warn(`[FRIDAY] ${modelId} provider down, trying next...`);
          continue;
        }
        throw new Error(msg || `HTTP ${res.status}`);
      }
      if (modelId !== CFG.openrModel) {
        const mb = document.getElementById('modelBadge');
        if (mb) mb.textContent = modelId.split('/')[1]?.split(':')[0]?.toUpperCase() + ' (FALLBACK)';
      }
      return data.choices?.[0]?.message?.content || 'No response received, boss.';
    } catch(e) {
      if (e.message.includes('Invalid API key') || e.message.includes('limit reached')) throw e;
      if (e.message.includes('fetch') || e.message.includes('network')) throw new Error('Network error — check your internet connection, boss.');
      console.warn(`[FRIDAY] ${modelId} error:`, e.message);
      continue;
    }
  }
  throw new Error('All free models are currently unavailable — try again in a few minutes, boss.');
}

async function testUplink() {
  try {
    const reply = await callOpenRouter('Say exactly: Uplink confirmed, boss. OpenRouter online.');
    history.splice(-2, 2);
    setStatus('ONLINE', true);
    addMsg('friday', reply);
    speakReply(reply);
