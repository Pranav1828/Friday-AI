...
const CFG = {
  ...
  openrModel:  localStorage.getItem('friday_openr_model') || 'google/gemini-2.0-flash-exp:free',
...
const OR_MODELS = [
  { id:'google/gemini-2.0-flash-exp:free',           label:'Gemini 2.0 Flash (free) ⭐ Recommended' },
  ...
  for (const modelId of tryModels) {
    try {
      ...
      if (!res.ok) {
        ...
        // Provider down or no endpoints — try next model
        if (res.status === 503 || res.status === 502 || msg.toLowerCase().includes('provider') || msg.toLowerCase().includes('no endpoints')) {
          console.warn(`[FRIDAY] Model ${modelId} failed (${res.status}), trying next...`);
          continue;
        }
        throw new Error(msg || `HTTP ${res.status}`);
      }
      // Success — update badge if we fell back
      ...
      if (e.message.includes('fetch')) throw e; // network error, don't retry
      console.warn(`[FRIDAY] Model ${modelId} error:`, e.message);
      continue;
    }
  }
  throw new Error('All models unavailable — check openrouter.ai/status, boss.');
}
